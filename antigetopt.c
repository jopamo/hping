...
static struct ago_exception {
    int (*tester)(void);
    char* msg;
} ago_exceptions[3] = {{NULL, NULL}, {NULL, NULL}, {NULL, NULL}};

static int ago_exception_bits[] = {AGO_EXCEPT0, AGO_EXCEPT1, AGO_EXCEPT2};

/* static functions */
static struct ago_optlist* ago_lookup(struct ago_optlist* list, char* arg, int* islong, int* amb);
...
chain_start:
    if (chain) {
        if (*chain == '\0')
            ...
            if ((opt = ago_lookup(list, chain, &islong, NULL)) == NULL)
                return AGO_UNKNOWN;
    ...
    while (*argv) {
        ...
        if (!endoptions && argv[0][0] == '-' && argv[0][1] != '\0') {
            ...
            if ((opt = ago_lookup(list, argv[0], &islong, &amb)) == NULL)
...
#define IERR_ERRSTRING                             \
    "internal error. ago_gnu_error() called with " \
    "a bad error code (%d)\n"
void ago_gnu_error(char* pname, int error) {
    ...
    switch (error) {
        case AGO_UNKNOWN:
            if (ago_optname)
                fprintf(stderr, UNK_LONG_ERRSTRING, ago_optname);
            else
                fprintf(stderr, UNK_SHORT_ERRSTRING, ago_optchar);
            break;
        case AGO_REQARG:
            if (ago_optname)
                fprintf(stderr, ARG_LONG_ERRSTRING, ago_optname);
            else
                fprintf(stderr, ARG_SHORT_ERRSTRING, ago_optchar);
            break;
        case AGO_AMBIG:
...
struct ago_optlist* ago_lookup(struct ago_optlist* list, char* arg, int* islong, int* amb) {
    int i;
...
ok:
    ...
    for (i = 0; i < 3; i++) {
        if ((list->ao_flags & ago_exception_bits[i]) && ago_exceptions[i].tester) {
            if (ago_exceptions[i].tester()) {
                if (ago_optname) {
                    fprintf(stderr, "%s `--%s'\n", ago_exceptions[i].msg, ago_optname);
                }
                else {
                    fprintf(stderr, "%s `-%c'\n", ago_exceptions[i].msg, ago_optchar);
                }
                exit(1);
            }
        }

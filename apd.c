...
#define BOGUS_SET_F(x)                                             \
    int(x)(struct ars_packet * pkt, int layer, char* f, char* v) { \
        return 0;                                                  \
    }
...
int ars_d_set_ipopt_rr(struct ars_packet* pkt, int layer, char* f, char* v) {
    ...
    if (strcasecmp(f, "optlen") == 0) {
        ...
        if (ARS_DONTTAKE(pkt->p_layer[layer].l_flags, ARS_TAKE_IPOPT_PTR)) {
    ...
    if (strcasecmp(f, "optlen") == 0) {
        ...
        if (strisnum(v)) {
            ...
            if (!strcasecmp(v, "tsonly"))
                ...
                ars_set_error(
                    pkt, "Invalid symbol for ip.ts flags: '%s' (use: tsonly, tsandaddr, prespec or a numerical value)",
                    v);
                return -ARS_INVALID;
            }
        }
        ...
        for (i = 0; i < num; i++) {
            ...
            if (p) {
                if (flags == ARS_IPOPT_TS_TSONLY) {
                    ars_set_error(pkt,
                                  "Gateway specified but ip.ts flags set to 'tsonly'. (Try flags=tsandaddr,data=...)");
                    return -ARS_INVALID;
                }
                *p = '\0';
                p++;
                ...
                if (flags == ARS_IPOPT_TS_TSANDADDR || flags == ARS_IPOPT_TS_PRESPEC) {
                    ars_set_error(pkt,
                                  "Gateway not specified in data for ip.ts, but flags set to 'tsandaddr' or 'prespec'. "
                                  "(Try flags=tsonly)");
                    return -ARS_INVALID;
                }
                ts = ars_atou(addrv[i]);
...
int ars_d_set_ipopt_sec(struct ars_packet* pkt, int layer, char* f, char* v) {
    ...
    if (strcasecmp(f, "optlen") == 0) {
        ...
        if (strlen(v) != 4) {
            ars_set_error(
                pkt,
                "Invalid ip.sec hrest field value of '%s'(should be four hex digits, like this: ...,hrest=252A,...)",
                v);
            return -ARS_INVALID;
        }
        ...
        if (strlen(v) != 6) {
            ars_set_error(
                pkt, "Invalid ip.sec tcc field value of '%s'(should be six hex digits, like this: ...,tcc=252A27,...)",
                v);
            return -ARS_INVALID;
        }
...
int ars_d_set_tcpopt_sack(struct ars_packet* pkt, int layer, char* f, char* v) {
    ...
    if (strcasecmp(f, "optlen") == 0) {
        ...
        for (i = 0; i < num; i++) {
            ...
            if (!p) {
                ars_set_error(pkt,
                              "Invalid syntax for tcp.sack blocks: '%s' (try "
                              "...tcp.sack(blocks=123342-10/12653-50/0-0/0-0)... )");
                return -ARS_INVALID;
            }
            *p = '\0';
            p++;
...
static char* ars_decode_hex(struct ars_packet* pkt, char* s, int* blen) {
    ...
    *blen = len / 2;
    ...
    while (*s) {
        ...
        if (x0 == 255 || x1 == 255) {
            ars_set_error(pkt, "Wrong byte for 'hex' data: '%c%c'", *s, *(s + 1));
            free(saved);
            return NULL;
        }
        *d++ = (x0 << 4) | x1;
...
int ars_d_set_data(struct ars_packet* pkt, int layer, char* f, char* v) {
    ...
    if (strcasecmp(f, "file") == 0) {
        ...
        if ((fd = open(v, O_RDONLY)) == -1) {
            ars_set_error(pkt, "Can't open the DATA file '%s': %s", v, strerror(errno));
            return -ARS_ERROR;
        }
...
int ars_d_build(struct ars_packet* pkt, char* t) {
    ...
    if (state != ARS_G_LEN_OR_PLUS && state != ARS_G_PLUS && state != ARS_G_OBRACE_OR_PLUS) {

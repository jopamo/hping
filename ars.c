...
int ars_compiler_abort(struct ars_packet* pkt, int layer) {
    return 0;
}
...
int ars_destroy(struct ars_packet* pkt) {
    ...
    free(pkt->p_error);
    for (j = 0; j < ARS_MAX_LAYER; j++) {
        if (pkt->p_layer[j].l_type != ARS_TYPE_NULL && pkt->p_layer[j].l_data != NULL)
...
int ars_add_generic(struct ars_packet* pkt, size_t size, int type) {
    ...
    if (size != 0) {
        ...
        if (pkt->p_default[type] != NULL) {
            memcpy(pkt->p_layer[layer].l_data, pkt->p_default[type], size);
        }
    }
...
int ars_compiler_ip(struct ars_packet* pkt, int layer) {
    ...
    if (ARS_DONTTAKE(flags, ARS_TAKE_IP_CKSUM)) {
        ...
        for (j = layer + 1; j < ARS_MAX_LAYER; j++) {
            if (pkt->p_layer[j].l_type != ARS_TYPE_IPOPT)
                break;
            err = ars_multi_cksum(&mc, ARS_MC_UPDATE, pkt->p_layer[j].l_data, pkt->p_layer[j].l_size);
            if (err != -ARS_OK)
                return err;
        }
...
int ars_compiler_ipopt(struct ars_packet* pkt, int layer) {
    ...
    if (layer != ARS_MAX_LAYER - 1 && pkt->p_layer[layer + 1].l_type == ARS_TYPE_IPOPT)
        return ARS_OK;

    /* Search the layer of the relative first IP option */
    j = layer - 1; /* We know that 'layer' is an IP option */
    while (j < ARS_MAX_LAYER && j >= 0 && pkt->p_layer[j].l_type == ARS_TYPE_IPOPT)
        j--;
    j++;
...
int ars_udptcp_cksum(struct ars_packet* pkt, int layer, u_int16_t* sum) {
    ...
    if (pkt->p_layer[j].l_type != ARS_TYPE_IP) {
        ars_set_error(pkt,
                      "TCP/UDP checksum requested, but IP header "
                      "not found");
        return -ARS_INVALID;
    }
    ...
    pseudo.protocol = (pkt->p_layer[layer].l_type == ARS_TYPE_TCP) ? ARS_IPPROTO_TCP : ARS_IPPROTO_UDP;
...
int ars_compiler_tcpopt(struct ars_packet* pkt, int layer) {
    ...
    if (layer != ARS_MAX_LAYER - 1 && pkt->p_layer[layer + 1].l_type == ARS_TYPE_TCPOPT)
        return ARS_OK;

    /* Search the layer of the relative first TCP option */
    j = layer - 1; /* We know that 'layer' is a tcp option */
    while (j < ARS_MAX_LAYER && j >= 0 && pkt->p_layer[j].l_type == ARS_TYPE_TCPOPT)
        j--;
    j++;
...
int ars_open_rawsocket(struct ars_packet* pkt) {
    ...
    if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, (char*)&one, sizeof(one)) == -1 ||
        setsockopt(s, IPPROTO_IP, IP_HDRINCL, (char*)&one, sizeof(one)) == -1) {
        close(s);
...
int ars_build_packet(struct ars_packet* pkt, unsigned char** packet, size_t* size) {
    ...
    while (j < ARS_MAX_LAYER && pkt->p_layer[j].l_type != ARS_TYPE_NULL) {
        memcpy((*packet) + offset, pkt->p_layer[j].l_data, pkt->p_layer[j].l_size);
...
int ars_bsd_fix(struct ars_packet* pkt, unsigned char* packet, size_t size) {
    ...
    if (pkt->p_layer[0].l_type != ARS_TYPE_IP || size < sizeof(struct ars_iphdr)) {

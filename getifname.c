...
#include "globals.h"
    ...
    !defined(__sun__) && !defined(__bsdi__) && !defined(__APPLE__)
...
int get_if_name(void) {
    ...
    if (ifname[0] == '\0') {
        if (get_output_if(&remote, &output_if_addr) == 0) {
            ...
            fprintf(stderr,
                    "Warning: Unable to guess the output "
                    "interface\n");
        }
    }
...
#elif defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__) || defined(__bsdi__) || defined(__APPLE__)
...
int get_if_name(void) {
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_data) {
            if (opt_debug)
                printf("DEBUG: (struct DATA) ");
        }
        else if (opt_debug)
            printf("DEBUG: (struct DATA is NULL) ");
        ...
        if ((ifa->ifa_flags & IFF_LOOPBACK) && (strncmp(saved_ifname, "lo0", 3))) { /* if loopback */
            if (opt_debug)
...
/* I don't know why NetBSD behavior is not the same */
        ...
        if (ifa->ifa_addr->sa_family == AF_INET) {
            ...
            strlcpy(ifstraddr, inet_ntoa(((struct sockaddr_in*)ifa->ifa_addr)->sin_addr), 1024);

            if ((saved_ifname[0] == 0) || (!strncmp(ifa->ifa_name, saved_ifname, 24)) ||
                (((struct sockaddr_in*)ifa->ifa_addr)->sin_addr.s_addr == output_if_addr.sin_addr.s_addr))
...
int get_output_if(struct sockaddr_in* dest, struct sockaddr_in* ifip) {
    ...
    if (setsockopt(sock_rt, SOL_SOCKET, SO_BROADCAST, &on, sizeof(on)) == -1) {
        if (opt_debug)
            perror(
                "DEBUG: [get_output_if] setsockopt(SOL_SOCKET, "
                "SO_BROADCAST");
        close(sock_rt);
        return -1;
    }

...
int hping_get_interfaces(struct hpingif* hif, int ilen) {
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        ...
        for (; ift; ift = ift->ifa_next) {
            if (ift->ifa_addr->sa_family == AF_INET && ift->ifa_addr && !strcmp(ifa->ifa_name, ift->ifa_name)) {
        ...
        /* Read the media status */
        {
            ...
            if (s != -1 && ioctl(s, SIOCGIFMEDIA, (caddr_t)&ifmr) != -1) {
...
int main(void) {
    ...
    for (i = 0; i < found; i++) {
        struct in_addr ia;
        printf("%-10.10s %-10d %-10d", ifaces[i].hif_name, ifaces[i].hif_mtu, ifaces[i].hif_index);
        printf("(");

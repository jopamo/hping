...
static struct ago_optlist hping_optlist[] = {{'c', "count", OPT_COUNT, AGO_NEEDARG},
...
int parse_options(int argc, char** argv) {
    ...
    while ((o = antigetopt(argc, argv, hping_optlist)) != AGO_EOF) {
        switch (o) {
            ...
            case OPT_INTERVAL:
                ...
                if (*ago_optarg == 'u') {
                    opt_waitinusec = TRUE;
                    usec_delay.it_value.tv_sec = usec_delay.it_interval.tv_sec = 0;
                    usec_delay.it_value.tv_usec = usec_delay.it_interval.tv_usec = atol(ago_optarg + 1);
                }
                else
            ...
            case OPT_FAST:
                ...
                opt_waitinusec = TRUE;
                usec_delay.it_value.tv_sec = usec_delay.it_interval.tv_sec = 0;
                usec_delay.it_value.tv_usec = usec_delay.it_interval.tv_usec = 100000;
                break;
            case OPT_FASTER:
                delay_changed = 1;
                opt_waitinusec = TRUE;
                usec_delay.it_value.tv_sec = usec_delay.it_interval.tv_sec = 0;
                usec_delay.it_value.tv_usec = usec_delay.it_interval.tv_usec = 1;
            ...
            case OPT_LSRR:
                ...
                if (lsr[0])
                    printf(
                        "Warning: erasing previously given "
                        "loose source route");
                lsr[0] = 131;
                break;
            case OPT_SSRR:
            ...
            case OPT_CS_WINDOW:
                ...
                if (cs_window < 30) {
                    fprintf(stderr, "clock skew window can't be < 30 sec.\n");
                    exit(1);
                }
                break;
            case OPT_CS_WINDOW_SHIFT:
                ...
                if (cs_window_shift < 1) {
                    fprintf(stderr, "clock skew window shift can't be < 1\n");
                    exit(1);
                }
                break;
            case OPT_CS_VECTOR_LEN:
                ...
                if (cs_vector_len < 1) {
                    fprintf(stderr, "clock skew packets per sample can't be < 1\n");
                    exit(1);
                }
                break;
        }
    ...
    if (targethost_set == 0 && !opt_listenmode)
        return -1;
    ...
    if (data_size + IPHDR_SIZE + TCPHDR_SIZE > 65535) {
        ...
        printf(
            "Option error: sorry, you can't set id and "
            "use safe protocol at some time\n");
        exit(1);
    }
    else if (opt_safe == TRUE && opt_datafromfile == FALSE && opt_listenmode == FALSE) {
        printf(
            "Option error: sorry, safe protocol is useless "
            "without 'data from file' option\n");
        exit(1);
    }
    else if (opt_safe == TRUE && opt_sign == FALSE && opt_listenmode == FALSE) {
        printf(
            "Option error: sorry, safe protocol require you "
            "sign your packets, see --sign | -e option\n");
        exit(1);
    }
    else if (opt_rand_dest == TRUE && ifname[0] == '\0') {
        printf(
            "Option error: you need to specify an interface "
    ...
    if (opt_scanmode && !delay_changed) {
        opt_waitinusec = TRUE;
        usec_delay.it_value.tv_sec = usec_delay.it_interval.tv_sec = 0;
        usec_delay.it_value.tv_usec = usec_delay.it_interval.tv_usec = 0;
    }

...
int ars_rapd_ip(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    adbuf_printf(dest, "fragoff=%u,", (ntohs(ip->frag_off) & 0x1FFF) << 3);
    if (!defip || (ip->frag_off & ARS_IP_MF) != (defip->frag_off & ARS_IP_MF))
        adbuf_printf(dest, "mf=%d,", (htons(ip->frag_off) & ARS_IP_MF) != 0);
    if (!defip || (ip->frag_off & ARS_IP_DF) != (defip->frag_off & ARS_IP_DF))
        adbuf_printf(dest, "df=%d,", (htons(ip->frag_off) & ARS_IP_DF) != 0);
    if (!defip || (ip->frag_off & ARS_IP_RF) != (defip->frag_off & ARS_IP_RF))
        adbuf_printf(dest, "rf=%d,", (htons(ip->frag_off) & ARS_IP_RF) != 0);
    ...
    adbuf_printf(dest, "cksum=0x%04x,", ip->check);
    {
        ...
        adbuf_printf(dest, "saddr=%u.%u.%u.%u,", x[0], x[1], x[2], x[3]);
        x = (unsigned char*)&ip->daddr;
        adbuf_printf(dest, "daddr=%u.%u.%u.%u", x[0], x[1], x[2], x[3]);
    }
    adbuf_printf(dest, ")+");
    return -ARS_OK;
}
...
int ars_rapd_ipopt(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    switch (ipopt.kind) {
        ...
        case ARS_IPOPT_SSRR: {
            ...
            adbuf_printf(dest, "ip.%s(ptr=%u,data=", optname, ipopt.un.rr.ptr);
            while (1) {
                unsigned char* x;

                if (ptr > 37 || ptr > (optlen - 3))
                    break;
                x = optp + ptr - 1;
                adbuf_printf(dest, "%u.%u.%u.%u/", x[0], x[1], x[2], x[3]);
                ptr += 4;
            }
            if (ptr > 4)
                adbuf_rtrim(dest, 1);
        ...
        case ARS_IPOPT_TIMESTAMP: {
            ...
            switch (flags) {
                case ARS_IPOPT_TS_TSONLY:
                    strflags = "tsonly";
                    break;
                case ARS_IPOPT_TS_TSANDADDR:
                    strflags = "tsandaddr";
                    break;
                case ARS_IPOPT_TS_PRESPEC:
                    strflags = "prespec";
                    break;
                default:
                    strflags = NULL;
            ...
            while (1) {
                ...
                if (ptr > 37 || ptr > (optlen - 4))
                    break;
                if (flags != ARS_IPOPT_TS_TSANDADDR && flags != ARS_IPOPT_TS_PRESPEC) {
                    ...
                    x = optp + ptr - 1;
                    memcpy(&ts, x + 4, 4);
                    adbuf_printf(dest, "%u@%u.%u.%u.%u/", ts, x[0], x[1], x[2], x[3]);
                    ptr += 8;
                }
            }
            if (ptr > 5)
...
int ars_rapd_icmp(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    if (icmp->type == ARS_ICMP_DEST_UNREACH || icmp->type == ARS_ICMP_TIME_EXCEEDED ||
        icmp->type == ARS_ICMP_PARAMETERPROB || icmp->type == ARS_ICMP_SOURCE_QUENCH) {
        adbuf_printf(dest, "unused=%lu,", (unsigned long)ntohl(icmp->un.gateway));
    }
    if (icmp->type == ARS_ICMP_ECHOREPLY || icmp->type == ARS_ICMP_ECHO || icmp->type == ARS_ICMP_TIMESTAMP ||
        icmp->type == ARS_ICMP_TIMESTAMPREPLY || icmp->type == ARS_ICMP_INFO_REQUEST ||
    ...
    if (icmp->type == ARS_ICMP_REDIRECT) {
        ...
        adbuf_printf(dest, "gw=%u.%u.%u.%u,", x[0], x[1], x[2], x[3]);
    }
    adbuf_rtrim(dest, 1);
    adbuf_printf(dest, ")+");
    return -ARS_OK;
...
int ars_rapd_tcp(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    if (tcp->th_flags & ARS_TCP_TH_FIN)
        adbuf_printf(dest, "f");
    if (tcp->th_flags & ARS_TCP_TH_SYN)
        adbuf_printf(dest, "s");
    if (tcp->th_flags & ARS_TCP_TH_RST)
        adbuf_printf(dest, "r");
    if (tcp->th_flags & ARS_TCP_TH_PUSH)
        adbuf_printf(dest, "p");
    if (tcp->th_flags & ARS_TCP_TH_ACK)
        adbuf_printf(dest, "a");
    if (tcp->th_flags & ARS_TCP_TH_URG)
        adbuf_printf(dest, "u");
    if (tcp->th_flags & ARS_TCP_TH_X)
        adbuf_printf(dest, "x");
    if (tcp->th_flags & ARS_TCP_TH_Y)
        adbuf_printf(dest, "y");
    adbuf_printf(dest, ",");
...
int ars_rapd_tcpopt(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    switch (tcpopt.kind) {
        ...
        case ARS_TCPOPT_MAXSEG:
            adbuf_printf(dest, "tcp.mss(size=%u)+", ntohs(tcpopt.un.mss.size));
            break;
        case ARS_TCPOPT_WINDOW:
            adbuf_printf(dest, "tcp.wscale(shift=%u)+", tcpopt.un.win.shift);
            break;
        ...
        case ARS_TCPOPT_SACK:
            adbuf_printf(dest, "tcp.sack(blocks=");
            {
                ...
                for (i = 0; i < blocks; i++) {
                    ...
                    adbuf_printf(dest, "%lu-%lu", ntohl(s_orig), ntohl(s_size));
                    if ((i + 1) != blocks)
                        adbuf_addchar(dest, '/');
                }
            }
        ...
        case ARS_TCPOPT_ECHOREQUEST: {
            ...
            adbuf_printf(dest, "tcp.echoreq(info=%lu)+", (unsigned long)ntohl(info));
        } break;
        case ARS_TCPOPT_ECHOREPLY: {
            __u32 info;
            memcpy(&info, tcpopt.un.echo.info, 4);
            adbuf_printf(dest, "tcp.echoreply(info=%lu)+", (unsigned long)ntohl(info));
        } break;
        case ARS_TCPOPT_TIMESTAMP: {
            __u32 tsval, tsecr;
            memcpy(&tsval, tcpopt.un.timestamp.tsval, 4);
            memcpy(&tsecr, tcpopt.un.timestamp.tsecr, 4);
            adbuf_printf(dest, "tcp.timestamp(val=%lu,ecr=%lu)+", (unsigned long)ntohl(tsval),
...
int ars_rapd_data(struct adbuf* dest, struct ars_packet* pkt, int layer) {
    ...
    if (ars_test_option(pkt, ARS_OPT_RAPD_HEXDATA)) {
        ...
        for (i = 0; i < dlen; i++) {
            ...
            if (isgraph(data[i]) && data[i] != '(' && data[i] != ')' && data[i] != '+' && data[i] != ',' &&
                data[i] != '=')

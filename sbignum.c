...
/* division */
static int mpz_divi_qr_raw(mpz_ptr q, mpz_ptr r, mpz_ptr z, mpz_atom_t* d, u_int32_t l);
...
#define _mpz_testbit(z, i) ((z)->d[(i) >> DIVATOMBITS_SHIFT] & ((mpz_atom_t)1 << ((i) & MODATOMBITS_MASK)))
...
int32_t mpz_cmp(mpz_ptr a, mpz_ptr b) {
    ...
    if (a->s == b->s) { /* same sign */
        if (a->s)
            return mpz_cmpabs(b, a); /* both negative */
...
int mpz_powm(mpz_ptr r, mpz_ptr b, mpz_ptr e, mpz_ptr m) {
    ...
    while (mpz_cmpabs_ui(E, 1) > 0) {
        if (_mpz_is_odd(E)) {
            if ((err = mpz_mul(r, r, B)) != SBN_OK)
                goto error;
            if ((err = mpz_mod(r, r, m)) != SBN_OK)
                goto error;
        }
        _mpz_self_rshift1(E); /* e = e / 2 */
        if ((err = mpz_mul(B, B, B)) != SBN_OK)
            goto error;
        if ((err = mpz_mod(B, B, m)) != SBN_OK)
            goto error;
    }
    if ((err = mpz_mul(r, r, B)) != SBN_OK)
        goto error;
...
int mpz_sqrt(mpz_ptr r, mpz_ptr z) {
    ...
    if ((err = mpz_realloc(s, atoms)) != SBN_OK)
        return err;
    if ((err = mpz_realloc(R, atoms)) != SBN_OK)
        return err;
    if ((err = mpz_realloc(X, atoms)) != SBN_OK)
        return err;
    if ((err = mpz_zero_realloc(r, atoms)) != SBN_OK)
        return err;
    for (; i >= 0; i--, b -= 2) {
        _mpz_setbit(R, b);

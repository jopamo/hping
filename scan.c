...
static void sender(struct portinfo* pi) {
    ...
    while (1) {
        ...
        if ((!recvd && opt_waitinusec && usec_delay.it_interval.tv_usec == 0 &&
...
static void receiver(struct portinfo* pi, int childpid) {
    ...
    while (1) {
        ...
        if (ip.protocol != IPPROTO_ICMP && memcmp(&ip.saddr, &remote.sin_addr, sizeof(ip.saddr)))
            continue;
        if (ip.protocol == IPPROTO_TCP) {
            ...
            if ((tcp.th_flags & TH_SYN) || opt_verbose) {
                printf("%5d %-11.11s: %s %3d %5d %5d %5d\n", sport, port_to_name(sport), flags, ip.ttl, ip.id,
                       ntohs(tcp.th_win), iplen);
                fflush(stdout);
            }
            pi[sport].active = 0;
        }
            ...
            printf("%5d:                      %3d %5d %5d   (ICMP %3d %3d from %s)\n", port, ip.ttl, iplen,
                   ntohs(ip.id), icmp.type, icmp.code, inet_ntoa(gwaddr));
        }
    }
}

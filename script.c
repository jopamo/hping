...
static int HpingResolveCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
...
static int HpingSendCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    struct ars_packet p;
    int nocompile = 0;
    Tcl_Obj* result;
...
static int HpingSendRawCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    int error;
    Tcl_Obj* result;
...
static int HpingRecvPackets(struct recv_handler* ra,
                            Tcl_Interp* interp,
                            Tcl_Obj* o,
                            int timeout,
                            int maxpackets,
...
static int __HpingRecvCmd(ClientData clientData,
                          Tcl_Interp* interp,
                          int objc,
                          Tcl_Obj* CONST objv[],
                          int rapd,
...
static int HpingRecvRawCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    return __HpingRecvCmd(clientData, interp, objc, objv, 0, 0);
}

static int HpingRecvCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    char* firstarg;
    int hexdata = 0;
...
/* hping getinterfaces */
static int HpingGetInterfacesCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    ...
    for (i = 0; i < found; i++) {
        ...
        Tcl_AppendStringsToObj(result, "{", ifaces[i].hif_name, " ", mtu, " ", NULL);
...
static int HpingGetOutIfAddrCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
...
static int HpingGetFieldCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
...
static int HpingHasFieldCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
...
static int HpingSetFieldCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    char *layer, *field, *value, *packet;
...
static int HpingDelFieldCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    char *layer, *field, *packet;
    ...
    if (packet[fstart - 1] == ',' && (packet[vend + 1] == ')' || packet[vend + 1] == ',')) {
        fstart--;
    }
...
static int HpingChecksumCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    Tcl_Obj* result;
    u_int16_t cksum;
    char* data;
    int len;
...
static int HpingSetFilterCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    struct recv_handler* ra;
...
static int HpingEventCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    struct recv_handler* ra;
    char* ifname;
    Tcl_Obj* result;
    int scriptlen;
    ...
    if (scriptlen != 0) {
        ...
        Tcl_CreateFileHandler(pcap_fileno(ra->rh_pcapfp), TCL_READABLE, HpingEventHandler, (void*)ra);
    }
    else {
        ra->rh_interp = NULL;
    }
...
static int HpingObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    int i = 0;
    char* scmd;
    Tcl_Obj* result;
...
struct Tcl_ObjType tclMpzType = {"mpz", FreeMpzInternalRep, DupMpzInternalRep, UpdateStringOfMpz, SetMpzFromAny};
...
int SetMpzFromAny(struct Tcl_Interp* interp, Tcl_Obj* objPtr) {
    ...
    if (mpz_set_str(t, s, 0) != SBN_OK) {
        ...
        Tcl_AppendStringsToObj(Tcl_GetObjResult(interp), "Invalid big number: \"", s,
                               "\" must be a relative integer number", NULL);
        return TCL_ERROR;
    }
    /* Allocate */
    mpzPtr = (mpz_ptr)ckalloc(sizeof(struct struct_sbnz));
...
static int BigBasicObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    Tcl_Obj* result;
    mpz_t res;
    mpz_ptr t;
    char* cmd;
...
static int BigCmpObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    Tcl_Obj* result;
    mpz_ptr a, b;
    int cmp, res;
    char* cmd;
    ...
    cmd = Tcl_GetStringFromObj(objv[0], NULL);
    if (Tcl_GetMpzFromObj(interp, objv[1], &a) != TCL_OK || Tcl_GetMpzFromObj(interp, objv[2], &b) != TCL_OK)
        return TCL_ERROR;
    cmp = mpz_cmp(a, b);
...
static int BigRandObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    Tcl_Obj* result;
    int len = 1;
    mpz_t r;
...
static int BigSrandObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    char* seed;
    int len;
...
static int BigPowObjCmd(ClientData clientData, Tcl_Interp* interp, int objc, Tcl_Obj* CONST objv[]) {
    Tcl_Obj* result;
    int mpzerr;
    mpz_t r;         /* result */
    ...
    if (Tcl_GetMpzFromObj(interp, objv[1], &b) != TCL_OK || Tcl_GetMpzFromObj(interp, objv[2], &e) != TCL_OK ||
...
static int HpingTcl_AppInit(Tcl_Interp* interp) {
    ...
    Tcl_CreateObjCommand(interp, "hping", HpingObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "+", BigBasicObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "-", BigBasicObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "*", BigBasicObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "/", BigBasicObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "%", BigBasicObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, ">", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, ">=", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "<", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "<=", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "==", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "!=", BigCmpObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "rand", BigRandObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "srand", BigSrandObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);
    Tcl_CreateObjCommand(interp, "**", BigPowObjCmd, (ClientData)NULL, (Tcl_CmdDeleteProc*)NULL);

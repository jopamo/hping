...
void send_ip(char* src,
    ...
    if (!opt_fragment) {
        ip->id = (src_id == -1) ? htons((unsigned short)rand()) : htons((unsigned short)src_id);
    }
    else /* if you need fragmentation id must not be randomic */
    {
        ...
        ip->id = (src_id == -1) ? htons(getpid() & 255) : htons((unsigned short)src_id);
    }

...
void send_ip_handler(char* packet, unsigned int size) {
    ...
    if (!opt_fragment) {
        ...
        if (opt_mf)
            fragment_flag |= MF; /* more fragments */
        if (opt_df)
            fragment_flag |= DF; /* dont fragment */
        send_ip((char*)&local.sin_addr, (char*)&remote.sin_addr, packet, size, fragment_flag, ip_frag_offset, ip_opt,
                ip_optlen);
    }
    else {
        ...
        while (1) {
            ...
            send_ip((char*)&local.sin_addr, (char*)&remote.sin_addr, packet + frag_offset, virtual_mtu, MF, frag_offset,
                    ip_opt, ip_optlen);
        ...
        send_ip((char*)&local.sin_addr, (char*)&remote.sin_addr, packet + frag_offset, remainder, NF, frag_offset,
                ip_opt, ip_optlen);
    }
}

...
int ars_seems_ip(struct ars_iphdr* ip, size_t size) {
    if (ip->version == 4 && ip->ihl >= 5 && (ip->ihl << 2) <= size && ars_check_ip_cksum(ip) == 1)
        return 1;
    return 0;
}
...
int (*ars_split_state_handler[])(struct ars_packet* pkt, void* packet, size_t size, int* state, int* len) = {
    NULL,          ars_split_ip,     ars_split_ipopt, ars_split_icmp,      ars_split_udp,
    ars_split_tcp, ars_split_tcpopt, ars_split_igrp,  ars_split_igrpentry, ars_split_data};
...
int ars_split_packet(void* packet, size_t size, int ipoff, struct ars_packet* pkt) {
    ...
    while (state != ARS_SPLIT_DONE) {
        ...
        error = ars_split_state_handler[state](pkt, packet + offset, size, &state, &len);
        if (error != -ARS_OK)
            return error;
        /* put off the link layer padding */
        if (pkt->p_layer_nr == 1 && pkt->p_layer[0].l_type == ARS_TYPE_IP) {
...
int ars_split_tcpopt(struct ars_packet* pkt, void* packet, size_t size, int* state, int* len) {
    ...
    if (tcpopt->kind == ARS_TCPOPT_EOL || tcpopt->kind == ARS_TCPOPT_NOP || tcpopt->kind == ARS_TCPOPT_SACK_PERM)
        optsize = 1;
    else
        optsize = tcpopt->len;

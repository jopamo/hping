...
void log_ip(int status, int sequence) {
    ...
    if (opt_relid)
        ...
        rel_id = 0;
    printf("len=%d ip=%s ttl=%d %sid%s%d ", ip_size, inet_ntoa(src), ip.ttl, (ntohs(ip.frag_off) ? "DF " : ""),
           (rel_id ? "=+" : "="), ip_id);
...
void log_icmp_ts(void* ts) {
    ...
    printf("ICMP timestamp: Originate=%u Receive=%u Transmit=%u\n", (unsigned int)ntohl(icmp_tstamp.orig),
           (unsigned int)ntohl(icmp_tstamp.recv), (unsigned int)ntohl(icmp_tstamp.tran));
    printf("ICMP timestamp RTT tsrtt=%lu\n\n", (long unsigned int)(get_midnight_ut_ms() - ntohl(icmp_tstamp.orig)));
}
...
void log_traceroute(void* packet, int size, int icmp_code) {
    ...
    retval = icmp_unreach_rtt(packet + ICMPHDR_SIZE, size - ICMPHDR_SIZE, &sequence, &rtt);
...
int recv_icmp(void* packet, size_t size) {
     ...
     * --------------------------- */
    if ((icmp.type == ICMP_ECHOREPLY || icmp.type == ICMP_TIMESTAMPREPLY || icmp.type == ICMP_ADDRESSREPLY) &&
     ...
     * ------------------------------------ */
        ...
        if (memcmp(&quoted_ip.daddr, &remote.sin_addr, sizeof(quoted_ip.daddr)) ||
...
int recv_udp(void* packet, size_t size) {
    ...
    if ((ntohs(udp.uh_sport) == dst_port) ||
        (opt_force_incdport && (ntohs(udp.uh_sport) >= base_dst_port && ntohs(udp.uh_sport) <= dst_port))) {
        recv_beep();
...
int recv_tcp(void* packet, size_t size) {
    ...
    if ((ntohs(tcp.th_sport) == dst_port) ||
        (opt_force_incdport && (ntohs(tcp.th_sport) >= base_dst_port && ntohs(tcp.th_sport) <= dst_port))) {
        recv_beep();
        ...
        if (opt_seqnum) {
            ...
            if (tmp >= old_th_seq)
                ...
                seq_diff = (4294967295U - old_th_seq) + tmp;
            old_th_seq = tmp;
            printf("%10lu +%lu\n", (unsigned long)tmp, (unsigned long)seq_diff);
            goto out;
        }
        ...
        log_ip(status, sequence);
        printf("sport=%d flags=%s seq=%d win=%d rtt=%.1f ms\n", ntohs(tcp.th_sport), flags, sequence, ntohs(tcp.th_win),
               ms_delay);

        if (opt_verbose) {
            printf("seq=%lu ack=%lu sum=%x urp=%u\n\n", (unsigned long)ntohl(tcp.th_seq),
                   (unsigned long)ntohl(tcp.th_ack), tcp.th_sum, ntohs(tcp.th_urp));
        }
...
int icmp_unreach_rtt(void* quoted_ip, int size, int* seqp, float* ms_delay) {
    ...
    if (qip.protocol == IPPROTO_TCP || qip.protocol == IPPROTO_UDP) {
        ...
        if ((size - quoted_iphdr_size) < 8 || icmp.type != ICMP_ECHO)
            return -1;
...
found:
    ...
    if (first_tstamp) {
        ...
        if (tsdiff > 0) {
            ...
            sec %= 60;
            printf(
                "  System uptime seems: %d days, %d hours, "
                "%d minutes, %d seconds\n",
                days, hours, minutes, sec);
        }
        if (hz > 0 && opt_clock_skew)
            clock_skew(hz, tstamp, rttms);
    }
    else {
        first_tstamp = tstamp;
        first_mstime = mstime();
